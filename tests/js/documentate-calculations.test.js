 ...
 * @return {string} HTML.
 */
function row( slug, i, j, values ) {
	const base = `tpl_fields[${ slug }][${ i }][conceptos][${ j }]`;
	return `
		<div class="documentate-subarray-item" data-subindex="${ j }">
			<input type="text" name="${ base }[concepto]" value="${ values.concepto || '' }">
			<input type="number" name="${ base }[cantidad]" value="${ values.cantidad || '' }">
			<input type="number" name="${ base }[unitario]" value="${ values.unitario || '' }">
			<input type="number" name="${ base }[total]" value="${ values.total || '' }">
		</div>`;
}
 ...
 * @param {string} slug    Repeater slug.
 * @param {number} i       Provider index.
 * @param {Object} values Provider values.
 * @param {Array}  rows   Concepto rows.
 * @return {string} HTML.
 */
function provider( slug, i, values, rows ) {
	...
	return `
		<div class="documentate-array-item" data-index="${ i }">
			...
			<input type="text" name="${ base }[proveedor]" value="${ values.provider || '' }">
			...
			<div class="documentate-subarray-field" data-subarray-field="conceptos">
				...
				<div class="documentate-subarray-items">${ rows.map( ( f, j ) => row( slug, i, j, f ) ).join( '' ) }</div>
			</div>
			<input type="number" name="${ base }[bruto]" value="">
			<input type="number" name="${ base }[igic]" value="${ values.igic || '' }">
			<input type="number" name="${ base }[irpf]" value="${ values.irpf || '' }">
 ...
 *
 * @param {string} slug        Repeater slug.
 * @param {Array}  providers Provider cards.
 * @return {string} HTML.
 */
function repeater( slug, providers ) {
	return `
		<div class="documentate-array-field" data-array-field="${ slug }">
			<button type="button" class="documentate-array-add">Add item</button>
			<div class="documentate-array-items">${ providers.join( '' ) }</div>
		</div>`;
}
...
const PROPOSAL = `
	<form id="post">
		${ repeater( 'servicios', [
			provider( 'servicios', 0, { provider: 'Acme', igic: '7', irpf: '' }, [
				{ concepto: 'Curso', cantidad: '2', unitario: '10.5' },
				{ concepto: 'Material', cantidad: '1', unitario: '1000' },
				{},
			] ),
		] ) }
		${ repeater( 'suministros', [ provider( 'suministros', 0, {}, [ {} ] ) ] ) }
		${ EXPENSE_FIELD }
	</form>`;
...
beforeEach( () => {
	document.body.innerHTML = PROPOSAL;
} );
...
describe( 'row and provider totals', () => {
	...
	it( 'keeps a row whose total was typed by hand and adds it to the bruto', () => {
		document.body.innerHTML = `
			<form id="post">
				${ repeater( 'expertos', [
					provider( 'expertos', 0, { provider: 'Ponente', igic: '', irpf: '' }, [
...
describe( 'summary card and gasto_numero', () => {
	it( 'builds the summary before gasto_numero and writes the grand total', () => {
		...
		expect( summary.nextElementSibling ).toBe( gasto );
		expect( summary.textContent ).toContain( 'Servicios (1)' );
		expect( summary.textContent ).toContain( 'Suministros (0)' );
		expect( summary.textContent ).toContain( 'Total de la propuesta' );
		expect( summary.querySelector( 'dd.documentate-resumen-total' ).textContent ).toBe( '1028,00 €' );
	...
	it( 'does nothing when the document has no provider repeaters', () => {
		...
		expect( window.documentateCalculations.recalculate() ).toEqual( [] );
	} );
} );
...
describe( 'helpers', () => {
	...
	it( 'formats amounts in es-ES with two decimals', () => {
		...
		expect( window.documentateCalculations.formatAmount( 1234.5 ) ).toBe( '1234,50 €' );
		expect( window.documentateCalculations.formatAmount( 0 ) ).toBe( '0,00 €' );
	} );
} );
