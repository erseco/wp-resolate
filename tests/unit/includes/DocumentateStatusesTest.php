<?php
/**
 * Tests for Documentate_Statuses.
 *
 * @package Documentate
 */

/**
 * @covers Documentate_Statuses
 */
class DocumentateStatusesTest extends WP_UnitTestCase {
	 ...
	 * @return WP_Post
	 */
	private function document_in( $status ) {
	...
	public function test_display_post_states() {
		...
		$this->assertSame( array(), Documentate_Statuses::display_post_states( array(), $post ) );
		$this->assertSame( array(), Documentate_Statuses::display_post_states( array(), null ) );
	}

	/**
	 * The metabox message follows the status and the flag each row keys on.
	 */
	public function test_metabox_message() {
		$this->assertSame(
	...
	public function test_quick_edit_removed_for_documents() {
		...
		$doc = self::factory()->post->create_and_get( array( 'post_type' => 'documentate_document' ) );
		$this->assertSame( array( 'edit', 'trash' ), array_keys( Documentate_Statuses::remove_quick_edit( $actions, $doc ) ) );

		$post = self::factory()->post->create_and_get();
		$this->assertSame( $actions, Documentate_Statuses::remove_quick_edit( $actions, $post ) );
	}
	...
	public function test_init_registers_filters() {
		...
		$this->assertSame( 10, has_filter( 'display_post_states', array( 'Documentate_Statuses', 'display_post_states' ) ) );
		$this->assertSame( 10, has_filter( 'post_row_actions', array( 'Documentate_Statuses', 'remove_quick_edit' ) ) );
