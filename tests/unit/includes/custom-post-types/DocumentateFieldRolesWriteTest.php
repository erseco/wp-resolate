 ...
 */
class DocumentateFieldRolesWriteTest extends WP_UnitTestCase {
	 ...
	 */
	public function test_saver_ignores_management_fields_posted_by_area() {
	 ...
	 *
	 * @dataProvider management_users
	 *
	 * @param string $who Property holding the user ID.
	 */
	public function test_saver_writes_management_fields_for( $who ) {
	 ...
	 */
	public function test_saver_refuses_a_management_slug_the_schema_does_not_declare() {
		...
		$summons = wp_insert_term( 'Convocatoria escritura ' . uniqid(), 'documentate_doc_type' );
		wp_set_post_terms( $other, array( (int) $summons['term_id'] ), 'documentate_doc_type', false );
		wp_set_post_terms( $other, wp_get_post_terms( $this->doc_id, 'category', array( 'fields' => 'ids' ) ), 'category', false );
		...
		( new Documentate_Document_Meta_Saver() )->save_meta_boxes( $other );

		$this->assertSame( '', get_post_meta( $other, 'documentate_field_numero_resolucion', true ) );
		$this->assertSame(
			'Valor de un tipo anterior',
			get_post_meta( $other, 'documentate_field_otro_campo', true ),
		...
		( new Documentate_Document_Meta_Saver() )->save_meta_boxes( $other );
		$this->assertSame( '999/2099', get_post_meta( $other, 'documentate_field_numero_resolucion', true ) );
	}

	/**
	 * The writer composes the stored gestión values when the área posts them.
	 */
	public function test_writer_keeps_stored_management_values_for_area() {
		$this->post_as( $this->area_id );
	 ...
	 * @param string $who Property holding the user ID.
	 */
	public function test_writer_takes_posted_management_values_for( $who ) {
		$this->post_as( $this->$who );
	 ...
	 */
	public function test_writer_drops_a_management_field_of_another_type() {
		$other = wp_insert_term( 'Propuesta escritura ' . uniqid(), 'documentate_doc_type' );
		...
		$this->post_as( $this->area_id );
		$_POST['documentate_doc_type'] = $other_id;
	 ...
	 */
	public function test_writer_ignores_management_values_forged_in_the_posted_content() {
		$this->post_as( $this->area_id );
	 ...
	 */
	public function test_writer_keeps_management_columns_of_an_area_block() {
		$this->post_as( $this->area_id );
	 ...
	 */
	public function test_writer_writes_management_columns_of_an_area_block_for_management() {
		$this->post_as( $this->management_id );
	 ...
	 */
	public function test_saver_keeps_management_columns_of_an_area_block() {
		$this->post_as( $this->area_id );
	 ...
	 */
	public function test_repeater_hides_management_columns_from_the_area() {
		wp_set_current_user( $this->area_id );
	...
	public function management_users() {
		return array(
			'management' => array( 'management_id' ),
			'administration' => array( 'admin_id' ),
		);
	}
}
