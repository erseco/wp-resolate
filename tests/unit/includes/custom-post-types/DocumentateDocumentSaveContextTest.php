...
class DocumentateDocumentSaveContextTest extends WP_UnitTestCase {
	...
	public function set_up(): void {
		...
		$this->otro_tipo_id = $this->crear_tipo(
			array(
		...
		wp_set_post_terms( $this->doc_id, array( $this->tipo_id ), 'documentate_doc_type', false );
	 ...
	 * @return int Term ID.
	 */
	private function crear_tipo( array $fields ) {
	...
	public function test_term_id_prefers_the_assigned_type() {
		$_POST['documentate_doc_type'] = $this->otro_tipo_id;

		$this->assertSame( $this->tipo_id, Documentate_Document_Save_Context::term_id( $this->doc_id ) );
	}
	...
	public function test_term_id_falls_back_to_the_posted_type() {
		...
		$this->assertSame( 0, Documentate_Document_Save_Context::term_id( $sin_tipo ) );
		$this->assertSame( 0, Documentate_Document_Save_Context::term_id( 0 ) );

		$_POST['documentate_doc_type'] = $this->otro_tipo_id;
		$this->assertSame( $this->otro_tipo_id, Documentate_Document_Save_Context::term_id( $sin_tipo ) );
	...
	public function test_existing_values_keeps_the_request_and_the_database_apart() {
		$forjado = Documents_Meta_Handler::build_structured_field_fragment( 'objeto', 'single', 'Del request' );

		$valores = Documentate_Document_Save_Context::existing_values(
			array( 'post_content' => $forjado ),
			$this->doc_id
		);

		$this->assertSame( 'Del request', $valores['request']['objeto']['value'] );
		$this->assertSame( 'Almacenado', $valores['stored']['objeto']['value'] );
	}
	...
	public function test_existing_values_falls_back_to_the_stored_map() {
		$valores = Documentate_Document_Save_Context::existing_values( array( 'post_content' => '' ), $this->doc_id );

		$this->assertSame( 'Almacenado', $valores['request']['objeto']['value'] );
		$this->assertSame( $valores['stored'], $valores['request'] );

		$vacio = Documentate_Document_Save_Context::existing_values( array(), 0 );
		$this->assertSame( array(), $vacio['stored'] );
		$this->assertSame( array(), $vacio['request'] );
	}
	...
	public function test_existing_values_reads_the_stored_post_content() {
		...
		$valores = Documentate_Document_Save_Context::existing_values( array(), $this->doc_id );

		$this->assertSame( 'En el contenido', $valores['stored']['objeto']['value'] );
	}
	...
	public function test_hidden_slugs_covers_the_posted_type_too() {
		...
		$ocultos = Documentate_Document_Save_Context::hidden_slugs(
			Documents_Meta_Handler::get_term_schema( $this->tipo_id )
		);

		$this->assertArrayHasKey( 'numero', $ocultos, 'Its own gestión field.' );
		$this->assertArrayHasKey( 'partida', $ocultos, 'And the one of the type the request names.' );
		$this->assertArrayNotHasKey( 'objeto', $ocultos );
	}

	/**
	 * Gestión documental hides nothing, and rows without a slug are skipped.
	 */
	public function test_hidden_slugs_is_empty_for_gestion() {
		wp_set_current_user( self::factory()->user->create( array( 'role' => 'administrator' ) ) );

		$this->assertSame(
			array(),
			Documentate_Document_Save_Context::hidden_slugs( Documents_Meta_Handler::get_term_schema( $this->tipo_id ) )
		);
		$this->assertSame(
			array(),
	...
	public function test_item_at_pairs_rows_by_index() {
		$filas = array( array( 'a' => '1' ), array( 'a' => '2' ) );

		$this->assertSame( array( 'a' => '2' ), Documentate_Document_Save_Context::item_at( $filas, '1' ) );
		$this->assertSame( array(), Documentate_Document_Save_Context::item_at( $filas, 'x' ) );
		$this->assertSame( array(), Documentate_Document_Save_Context::item_at( $filas, 7 ) );
