...
class Documentate_Estados {
	...
	public static function init() {
		...
		add_filter( 'post_row_actions', array( __CLASS__, 'quitar_quick_edit' ), 10, 2 );
	}
	 ...
	 *
	 * @param string $clave           Status key (draft, en_gestion, pending, publish, archived).
	 ...
	 */
	public static function mensaje_metabox( $clave, $is_admin, $con_gestion, $puede_modificar ) {
		$mensajes = array(
			'publish' => array(
				'success',
				'lock',
		...
		list( $tipo, $icono, $variante, $textos ) = $mensajes[ $clave ];

		return array( $tipo, $icono, $textos[ $variante ? 1 : 0 ] );
	}
	 ...
	 *
	 * @param string $etiqueta Status label.
	 * @return array<string|int,string|null>
	 */
	private static function label_count( $etiqueta ) {
		$texto = $etiqueta . ' <span class="count">(%s)</span>';
	 ...
	 * @return string[]
	 */
	public static function quitar_quick_edit( $actions, $post ) {
		if ( $post instanceof WP_Post && self::POST_TYPE === $post->post_type ) {
