...
class Documentate_Ficheros {
	 ...
	 *                safe to read.
	 */
	public static function ruta_de_adjunto( $attachment_id ) {
		$attachment_id = (int) $attachment_id;
		if ( $attachment_id <= 0 ) {
			return '';
		}

		return self::ruta_dentro_de_subidas( (string) get_attached_file( $attachment_id ) );
	}

	/**
	 * The same path, resolved and checked against the uploads directory.
	 *
	 * @param string $ruta Candidate path.
	 * @return string Absolute path, or an empty string.
	 */
	public static function ruta_dentro_de_subidas( $ruta ) {
		$ruta = (string) $ruta;
		...
		$base = isset( $subidas['basedir'] ) ? realpath( (string) $subidas['basedir'] ) : false;
	 ...
	 * on disk.
	 *
	 * @param string $nombre File name as a person wrote it.
	 * @return string Empty string when nothing usable is left.
	 */
	public static function nombre_para_cabecera( $nombre ) {
		return sanitize_file_name( (string) $nombre );
	}
}
